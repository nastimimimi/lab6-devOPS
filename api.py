...
class GeneticScheduler:
    ...
    def _count_conflicts(self, individual):
        ...
        for tid in range(self.data.teachers_count):
            for day in range(self.data.days_count):
                s, e = day * self.data.pairs_per_day, (day+1) * self.data.pairs_per_day
                day_cl = [x for x in individual if x['teacher_id'] == tid and s <= x['slot'] < e]
                if len(day_cl) > 4:
                    total += len(day_cl) - 4
        return total
    ...
    def _count_pref_deviation(self, individual):
        return sum(min(abs(e['slot'] - p) for p in self.data.teachers[e['teacher_id']]['preferred'])
                   for e in individual
                   if e['slot'] not in self.data.teachers[e['teacher_id']]['preferred'])
    ...
    def run(self):
        ...
        for gen in range(self.max_generations):
            ...
            population = sorted(new_pop + selected, key=self._fitness)[:self.pop_size]
            cur_score  = self._fitness(population[0])
...
def calculate():
    ...
    pop_size        = to_int(body.get('pop_size'),         50, 10, 500)
    max_generations = to_int(body.get('max_generations'), 200,  1, 2000)
